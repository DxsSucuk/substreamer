 ...
 *
 * A batch does not reach the engine when it is called: the Jest SQLite substitute
 * parks it on the lock and starts it from a `setImmediate`, one at a time, exactly
 * as op-SQLite's `enhanceDB` does (`src/db/testing/opSqliteBetterSqlite3.ts`). So N
 * un-awaited writes need N macrotasks, and a single `setTimeout(0)` settles only the
 * first — awaiting the write is what production does, and this is for the tests that
