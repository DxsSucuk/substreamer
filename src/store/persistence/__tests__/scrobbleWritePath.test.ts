 ...
 * seam: a live insert now writes the five `scrobble_*` child rows alongside the
 * parent, and writes an EMPTY `song_json`.
 *
...
const envelopeOf = (id: string): string | undefined =>
  realDb.getFirstSync<{ song_json: string }>('SELECT song_json FROM scrobble_events WHERE id = ?;', [
    id,
  ])?.song_json;
...
describe('insertScrobble — child rows written, envelope not', () => {
  ...
  it('stores an empty song_json', () => {
    expect(envelopeOf('sc-1')).toBe('');
  });
